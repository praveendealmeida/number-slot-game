...
| Payments  | Demo sandbox — buying a ticket debits the buyer's in-app wallet directly (`src/app/api/games/[gameId]/tickets/route.ts`), no external gateway |
...
Buying slots debits the buyer's in-app wallet directly — there is **no external payment gateway** right now:

1. The buyer selects slots and hits Buy.
2. `POST /api/games/[gameId]/tickets` runs one Prisma transaction that: locks the game row, checks slot availability, checks the buyer's `Wallet.balance`, debits it, writes a `Transaction` (`type: "TICKET_PURCHASE"`), and creates the tickets as `COMPLETED` — all atomically. If the balance is too low, nothing is charged and the slots aren't reserved.
3. No webhook, no polling, no held/`PENDING` state — the purchase either fully succeeds or fully fails in one request.

Wallets are funded by `DEMO_MODE`'s Rs 5,000 top-up (`src/services/daily-game.ts`) or by an admin manually crediting a user's wallet from the admin dashboard. **This is intentionally temporary** — swap in a real payment gateway in `src/app/api/games/[gameId]/tickets/route.ts` before this app should handle real money. The previous Chain2Pay integration (hosted checkout, webhook, status polling) is a reasonable reference for that shape if you reintroduce a gateway later — see git history.
...
- **Payments are a demo sandbox, not a real gateway.** Buying a ticket debits the buyer's wallet directly and completes instantly — see [Payments (demo sandbox)](#payments-demo-sandbox). Wire in a real gateway before this app should handle real money.
